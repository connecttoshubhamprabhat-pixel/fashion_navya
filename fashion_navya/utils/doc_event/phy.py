...
@frappe.whitelist()
def get_items_new(warehouse=None,item_group=None,project=None,max='0',min='0'):
	max=int(max)
	min=int(min)
	if not warehouse:
		return
	...
	for i in items:
		if max==0:
			...
			get_price=frappe.db.sql("""select name  from `tabItem Price`  where item_code='{}' and price_list_rate between {} and {}   """.format(i,min,max),as_dict=1)
			if len(get_price)!=0:
				d={}
				qty=[0]
				d['item_code']=i
...
def consolidated_entry(items=None):
	...
	if items:
		...
		d_1={"doctype":"Physical Stock Count","consolidated":1}
		aqty=[0]
		sqty=[0]
		diff_mins=[0]
		itemdoc=frappe.get_doc(d)
		physical=frappe.get_doc(d_1)
		for  i in items:
			doc=frappe.get_doc("Physical Stock Count",i)
			...
			for j in doc.items:
				...
				if j.dqty<0:
					row_1 = itemdoc.append("items_d", {})
					row_1.item_code=j.item_code
					row_1.item_name=j.item_name
					row_1.sqty=j.sqty
					row_1.aqty=j.aqty
					row_1.warehouse=j.warehouse
					row_1.dqty=j.dqty
					row_1.price=j.price
					row_1.image=j.image
					row_1.remarks=j.remarks
					dabs=abs(j.dqty)
					diff_mins.append(dabs)
				
				if j.dqty<0:
					row_2 = itemdoc.append("items_c", {})
					row_2.item_code=j.item_code
					row_2.item_name=j.item_name
					row_2.sqty=0
					row_2.aqty=j.aqty
					row_2.warehouse=j.warehouse
					row_2.dqty=0
					row_2.price=j.price
					row_2.image=j.image
					row_2.remarks=j.remarks
				
				if j.dqty<0:
					row_3 = physical.append("items", {})
					row_3.item_code=j.item_code
					row_3.item_name=j.item_name
					row_3.sqty=0
					row_3.aqty=j.aqty
					row_3.warehouse=j.warehouse
					row_3.dqty=0
					row_3.price=j.price
					row_3.image=j.image
					row_3.remarks=j.remarks
				
					



		#physical.insert(ignore_permissions=True)
...
@frappe.whitelist()
def update_after_submits_all(doc,method):
	frappe.msgprint(".")
	for i in doc.items_c:
		diff=i.sqty-i.aqty
		i.set("dqty",0)
		i.set("dqty",diff)
...
@frappe.whitelist()
def update_location(doc,method):
	if doc.warehouse:
		w=frappe.get_doc("Warehouse",doc.warehouse)
		if w.parent_warehouse in ['Santushti - NAVYA']:
			doc.set("location","Santushti")

		if w.parent_warehouse in ['Sainik Farm - NAVYA']:
			doc.set("location","Sainik Farm")

		if w.parent_warehouse in ['Pune - NAVYA']:
			doc.set("location","Pune")

		if w.parent_warehouse in ['Libberhedi Raw Material - NAVYA']:
			doc.set("location","Libberhedi")
