...
def get_columns():
    return [
        {"label": _("Shop"), "fieldtype": "Data", "fieldname": "shop", "width": 150},
        {"label": _("Initial Stock"), "fieldtype": "Float", "fieldname": "istock", "width": 150},
        {"label": _("Today Received"), "fieldtype": "Float", "fieldname": "rstock", "width": 150},
        {"label": _("Today Deliver"), "fieldtype": "Float", "fieldname": "dstock", "width": 150},
        {"label": _("Today Customer Return"), "fieldtype": "Float", "fieldname": "rnstock", "width": 150},
        {"label": _("Today Manual Return"), "fieldtype": "Float", "fieldname": "mstock", "width": 150},
        {"label": _("Total Stock"), "fieldtype": "Float", "fieldname": "stock", "width": 150},
    ]

def get_stock_entry_qty_today(group_warehouse, from_date, to_date):
    child_warehouses = get_child_warehouses(group_warehouse)
    qty_list = [0]
    for w in child_warehouses:
        qty = get_se_balance(w, from_date, to_date)
        if qty > 0:
            qty_list.append(qty)
    return sum(qty_list)
...
def get_se_balance(w, from_date, to_date):
    sql_query = """
        ...
        WHERE sed.t_warehouse = %s
            ...
            AND DATE(se.modified) BETWEEN %s AND %s
    """
    get_se = frappe.db.sql(sql_query, (w, from_date, to_date), as_dict=True)
    if get_se:
        return sum([d.balance for d in get_se if d.balance > 0])
    return 0
        ...
        for dn in get_dn:
            ...
            for q in dndoc.items:
                if q.against_sales_order:
                    ...
                    """, (q.against_sales_order, shops_dict.get(w)), as_dict=True)
                    if get_so:
...
def get_stock_ledger_entries(from_date, to_date, child_warehouses):
    ...
    query = f"""
        SELECT item_code, warehouse, posting_date, SUM(qty_after_transaction) as qty_after_transaction
        FROM `tabStock Ledger Entry`
        WHERE posting_date BETWEEN %s AND %s AND is_cancelled = 0 AND warehouse IN ({warehouse_placeholders})
...
def get_today_sales_return_qty():
    ...
    query = """
        SELECT SUM(total_qty) as total_return_qty
        FROM `tabSales Invoice`
        WHERE posting_date = %s AND is_return = 1 AND docstatus = 1
    """
    result = frappe.db.sql(query, today, as_dict=True)
    return result[0].total_return_qty if result and result[0].total_return_qty else 0

def get_stock_entry_qty_today_return(group_warehouse, from_date, to_date):
    child_warehouses = get_child_warehouses(group_warehouse)
    qty_list = [0]
    for w in child_warehouses:
        qty = get_se_balance_return(w, from_date, to_date)
        if qty > 0:
            qty_list.append(qty)
    return sum(qty_list)
